...
(C) 2011-2013 - Boxalino AG
*/


(function() {
  (function() {
    var accountId, api, buildKVL, buildMultiValuedKey, buildUri, bxs, bxv, cookieCache, cookieDomain, cookiePath, decode, e, encode, every, features, filter, forEach, getCookie, getParam, hasCookie, hash, helpers, i, identity, isSpace, item, k, loadingTask, log, makeuid, map, millis, parseCookie, parseKVL, parseUri, parseUrl, pkeys, plkeys, queue, reduce, removeCookie, seconds, sendNextEvent, setCookie, skeys, some, support, topw, trand, trim, v, writeMarkers, _bxs, _bxv, _i, _j, _len, _len1, _ref, _ref1, _ref2, _ref3, _ref4;
    ...
    api = {
      ...
      options: {
        ...
        timeout: 1000,
        tracker: '//cdn.bx-cloud.com/frontend/analytics/en/track'
      },
      ready: function() {
      ...
      push: function(cmd) {
        var callback, e, fn, promise;
        if (!((cmd != null ? cmd.length : void 0) > 0)) {
          return;
        }
        fn = api[cmd.shift()];
        if (cmd.length > 0 && typeof cmd[cmd.length - 1] === 'function') {
          callback = cmd.pop();
        }
        try {
    ...
    forEach = map = function(a, f) {
      var i, item, _i, _len, _results;
      _results = [];
      for (i = _i = 0, _len = a.length; _i < _len; i = ++_i) {
        item = a[i];
        _results.push(f(item, i, a));
      }
      return _results;
    };
    every = function(a, f) {
    ...
    reduce = function(a, f, init) {
      var i, item, value, _i, _len;
      value = init;
      for (i = _i = 0, _len = a.length; _i < _len; i = ++_i) {
        item = a[i];
        value = f(value, item, i, a);
      }
      return value;
    };
    isSpace = function(c) {
    ...
    hash = function(str) {
      ...
      h = 997;
      while (i < str.length) {
        h = Math.abs((h * 283 ^ str.charCodeAt(i)) & 0xffffffff);
        i += 1;
      }
      return h;
    };
    makeuid = function(items) {
      var item;
      return ((function() {
        ...
        for (_i = 0, _len = items.length; _i < _len; _i++) {
          item = items[_i];
          _results.push(parseInt(item).toString(16));
        }
        return _results;
      })()).join('.');
    };
    helpers.parseKVL = parseKVL = function(s1, s2, str) {
      var i, item, kvl, _i, _len, _ref;
      if (str == null) {
        str = '';
      }
      kvl = {};
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        kvl[trim(item.slice(0, i))] = trim(item.slice(i + 1));
      }
      return kvl;
    };
      ...
      return ((function() {
        ...
        for (k in values) {
          ...
          if (v && values.hasOwnProperty(k)) {
            _results.push((encodingFunction(key)) + s2 + (encodingFunction(v)));
          } else {
      ...
      return ((function() {
        ...
        for (k in kvl) {
          ...
          if (Object.prototype.toString.call(v) === "[object Array]") {
            _results.push(buildMultiValuedKey(k, v, s1, s2));
          } else if (v) {
            _results.push((encodingFunction(k)) + s2 + (encodingFunction(v)));
          } else {
            _results.push(void 0);
          }
        }
    ...
    trand = function() {
      ...
      while (dt === 0) {
        dt = millis() - t;
        x = Math.abs((dt + 97) * x & 0xffffffff);
      }
      return x;
    };
    helpers.parseUri = parseUri = function(uri) {
      var i, k, parameters, path, v, _ref;
      if (uri == null) {
        uri = '';
      }
      path = uri.toString();
      ...
      i = path.indexOf('?');
      if (i >= 0) {
        _ref = parseKVL('&', '=', path.slice(i + 1));
        for (k in _ref) {
          v = _ref[k];
          parameters[decode(k)] = decode(v.replace('+', ' ', 'g'));
        }
        path = path.slice(0, i);
      }
      return {
    ...
    /*
    	The following method creates tracking url with all parameters that tracker will receive
    */

    helpers.buildUri = buildUri = function(uri, parametersOverride, hashOverride) {
      var k, parameters, path, v, _ref;
      _ref = parseUri(uri), path = _ref.path, parameters = _ref.parameters, hash = _ref.hash;
    ...
    api.support = support = {
      ...
      storage: (topw.localStorage != null) && (topw.sessionStorage != null)
    };
    if (!support.cookie) {
      try {
        setCookie('bxanalytics-test', '1');
        if (getCookie('bxanalytics-test') === 1) {
          removeCookie('bxanalytics-test');
          support.cookie = true;
        }
      } catch (_error) {
        e = _error;
        if (log != null) {
          log('check cookie support', e);
        }
      }
    }
    bxv = {};
    bxs = {};
    if (support.cookie) {
      bxv.r = getCookie('cemv', '');
      bxs.r = getCookie('cems', '');
    }
    if (support.storage && (!bxv.r || bxv.r.length === 0) && (!bxs.r || bxv.r.length === 0)) {
      bxv.r = window.localStorage.getItem('cemv');
      bxs.r = window.sessionStorage.getItem('cems');
    }
    bxv.v = 2;
    try {
      if (!bxv.r || bxv.r.length === 0) {
        bxv.r = makeuid([hash(topw.navigator.userAgent), trand(), Math.random() * 1000, millis()]);
        bxv.t = seconds();
      }
      if (bxv.i != null) {
        bxv.i = parseInt(bxv.i) + 1;
      } else {
        bxv.i = 1;
      }
    } catch (_error) {
      e = _error;
      if (log != null) {
        log('build visitor marker', e);
      }
    }
    bxs.v = 2;
    try {
      if (!bxs.r || bxs.r.length === 0) {
        bxs.r = makeuid([hash(topw.navigator.userAgent), trand(), Math.random() * 1000, millis()]);
        bxs.t = seconds();
      }
      if (bxs.i != null) {
        bxs.i = parseInt(bxs.i) + 1;
      } else {
        bxs.i = 1;
      }
    } catch (_error) {
    ...
    try {
      ...
      if (((_ref = features.referrer.host) != null ? _ref.indexOf(topw.document.domain) : void 0) < 0) {
        if (((_ref1 = features.referrer.host) != null ? _ref1.length : void 0) > 0) {
          bxs.rh = features.referrer.host;
        }
        if (((_ref2 = features.referrer.path) != null ? _ref2.length : void 0) > 0) {
          bxs.rp = features.referrer.path;
        }
        for (k in skeys) {
          v = skeys[k];
          ...
          if (some(v, function(x) {
            ...
            bxs.rq = features.referrer.parameters[k];
            break;
          }
        }
        for (k in pkeys) {
        ...
        for (_i = 0, _len = plkeys.length; _i < _len; _i++) {
          k = plkeys[_i];
          if (((_ref3 = features.location) != null ? _ref3.parameters[k] : void 0) == null) {
            continue;
          }
          features.source.name = 'paid';
    ...
    _bxv = '';
    _bxs = '';
    writeMarkers = function() {
      _bxv = {};
      _bxs = {};
      for (k in bxv) {
        v = bxv[k];
        _bxv[encode(k)] = encode(v);
      }
      for (k in bxs) {
        v = bxs[k];
        _bxs[encode(k)] = encode(v);
      }
      _bxv = _bxv.r;
      _bxs = _bxs.r;
      try {
        if (support.cookie) {
          setCookie('cemv', _bxv, new Date(millis() + api.options.maxage));
          setCookie('cems', _bxs);
        }
        if (support.storage) {
          window.localStorage.setItem('cemv', _bxv);
          return window.sessionStorage.setItem('cems', _bxs);
        }
      } catch (_error) {
        e = _error;
        if (log != null) {
          return log('write markers', e);
        }
      }
    };
    writeMarkers();
    ...
    api.getVisitorKey = function() {
      return _bxv;
    };
    api.getSessionKey = function() {
      return _bxs;
    };
    ...
    queue = [];
    loadingTask = null;
    sendNextEvent = function() {
      var img, notifier, onError, onLoad, task;
      loadingTask = null;
      if (!(queue.length > 0)) {
        return;
      }
      try {
        loadingTask = task = queue.shift();
        img = document.createElement('img');
        onLoad = null;
        onError = null;
        notifier = function(success) {
          return function() {
            var callback, _j, _len1, _ref4;
            ...
            try {
              if (img.removeEventListener) {
                ...
                img.detachEvent('onload', onLoad);
                img.detachEvent('onerror', onError);
              }
              _ref4 = task.callbacks;
        ...
        onLoad = notifier(true);
        onError = notifier(false);
        if (api.options.timeout > 0) {
          task.timeout = setTimeout(onError, api.options.timeout);
        }
    ...
    api.trackEvent = function(name, parameters) {
      var register, task;
      if (parameters == null) {
        parameters = {};
      }
      ...
      parameters._a = accountId;
      parameters._t = millis();
      parameters._bxv = _bxv;
      parameters._bxs = _bxs;
      parameters._ev = name;
      parameters._rf = topw.document.referrer.toString();
      ...
      if ((parameters.choiceId == null) && (parameters.variantId == null) && (parameters.visitorId == null)) {
        parameters.choiceId = getParam('ba_choice_id', null);
        parameters.variantId = getParam('ba_variant_id', null);
        parameters.visitorId = getParam('ba_visitor_id', null);
      }
      task = {
        complete: false,
        success: false,
        url: buildUri(api.options.tracker, parameters),
        callbacks: []
      };
      queue.push(task);
      if (loadingTask == null) {
      ...
      return register = function(callback) {
        if (task.complete) {
    ...
    api.trackPageView = function(uri, page, variant, parameters) {
      var _ref4, _ref5;
      if (page == null) {
        page = null;
      }
      ...
      if (uri == null) {
        uri = buildUri((_ref4 = features.location) != null ? _ref4.path : void 0, (_ref5 = features.location) != null ? _ref5.parameters : void 0);
      }
      parameters.id = uri;
      parameters.q = page;
      parameters.p = variant;
    ...
    api.trackPurchase = function(total, currency, products, orderId, parameters) {
      ...
      for (i = _j = 0, _len1 = products.length; _j < _len1; i = ++_j) {
        product = products[i];
    ...
    api.trackLink = function(ev, parameters) {
      ...
      api.trackMouse(ev, parameters)(function() {
        return window.top.location = target.href;
      });
